...
def make_ics_for_dataframe(df):
    ...
    for _, r in df.iterrows():
        events.append(make_ics_for_row(r).decode("utf-8").splitlines()[3: -1])  # pull VEVENT block
    # stitch calendar
    ics = "BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//Project Tracker Pro//EN\n"
    for _, r in df.iterrows():
        ics += "BEGIN:VEVENT\n"
...
with st.expander("🖨️ Print / Export / Calendar"):
    ...
    if len(filtered) > 0:
        ...
        if st.button("📅 Download .ics for selected project"):
            ics_bytes = make_ics_for_row(df.loc[sel])
            st.download_button("Download ICS", data=ics_bytes, file_name=f"{df.loc[sel,'Project Code']}.ics", mime="text/calendar", key="dl_single_ics")
